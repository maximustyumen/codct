...
//////////////////////////////////////////////////////////////////////////
class FragmentShaderGeneratorPSSM : public osgShadow::ParallelSplitShadowMap::FragmentShaderGenerator {
public:
...
_LightSource(ls)
{
	...
	if ( _SSAO_type == SSAO::DEFAULT_SSAO  ) {
		osg::ref_ptr<osg::Shader> fragment_shader = new osg::Shader(osg::Shader::FRAGMENT,
			...
			"	gl_FragColor.rgb = colorText.rgb*ssao;\n"\
			"	gl_FragColor.rgb *= shadow.rgb ;\n"\
			"	//gl_FragColor.rgb =  colorText.rgb*ssao;\n"\
			"	gl_FragColor.a = colorText.a;\n"\
			"}"
			);
		//gl_FragColor.rgb = colorText.rgb  * 1;\n"
		//outColor = screenColor + mix(vec4(0.2), vec4(pow(AO, 2.0)), 1.0);
		//gl_FragColor.rgb = mix(colorText, shadow, 0.5).rgb ;\n"
		...
		osg::ref_ptr<osg::Shader> fragment_shader = new osg::Shader(osg::Shader::FRAGMENT,
			...
			"	gl_FragColor = shadow*gl_FragColor;\n"\
 			"	gl_FragColor.a = colorText.a;\n"\
			"}"
			//"	gl_FragColor.rgb += texture2D(text0, samplerRTSceneCoord).rgb ;\n"
			);
		program->addShader(fragment_shader.get());
